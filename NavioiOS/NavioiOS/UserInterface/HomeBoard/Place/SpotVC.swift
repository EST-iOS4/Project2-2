...
class SpotVC: UIViewController {
    ...
    private var heartButtons: [Int: UIButton] = [:] // index -> heart button
    ...
    private func setUpCombine() {
        // Bind each place's isLiked to its corresponding heart button
        for (index, place) in spotRef.places.enumerated() {
            place.$isLiked
                .receive(on: DispatchQueue.main)
                .sink { [weak self] liked in
                    self?.updateHeart(for: index, liked: liked)
                }
                .store(in: &cancellables)

            // Set initial state
            updateHeart(for: index, liked: place.isLiked)
        }
    }
    ...
    private func updateHeart(for index: Int, liked: Bool) {
...
//        let filteredPlaces = relatedPlaces.filter { $0 != placeName }
        ...
        // MARK: - PlaceInfo로 이동
        let placeInfoVC = PlaceVC(selectedPlace)
    ...
    @objc private func heartButtonTapped(_ sender: UIButton) {
        ...
        guard spotRef.places.indices.contains(index) else { return }
        let place = spotRef.places[index]
        place.toggleLike() // This will trigger Combine to update the button
    }
